    ...
    return (
      <>
        {user.user ? (
          <>
            <Link
              className="w-full flex flex-col justify-center items-center text-sm md:text-base hover:text-red-500"
              to={"/"}
              onClick={toggleMenu}
            >
              <BsHouse size={25} />
            ...
            <li className="w-full flex">
              <BotonLink to={"/productosIQ"} onClick={toggleMenu}>
                IQ Productos
              </BotonLink>
            </li>
            ...
            <li className="w-full flex justify-center items-center">
              <Link
                className=" bg-zinc-600  p-2 rounded-md text-center hover:text-cyan-400 block w-full"
                to={"/"}
                onClick={() => logout()}
              >
                Logout
  ...
  return (
    <nav className="container relative flex text-sm md:text-base justify-between items-start md:items-center bg-neutral-700 py-2 px-4 rounded-md">
      ...
      <div className="flex collapse md:visible justify-end items-center">
        ...
        <button
          className="flex flex-col justify-start visible md:hidden"
          onClick={toggleMenu}
        >
      ...
      {verMenu && (
        <ul className="absolute right-0 top-20 flex border border-neutral-600  p-2 items-center gap-3 
